...
app.put('/books/:id', async (req, res) => {
    try{
        if(!req.body.title || !req.body.author || !req.body.publishYear){
            return res.status(404).json( {message : "Enter the required fields : Title, author, publish year"} );
        }
        ...
        if(!book){
            return res.status(404).json( {message : `The book with id : ${id} is not found`});
        }
